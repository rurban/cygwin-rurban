...
rsync_up_target="xarch:software/cygwin/release/gis"
...
conf() {
  ...
  cp ${srcdir}/config.log ${srcinstdir}/ )
...
pkg() {
  (cd ${instdir} && \
  # geos
  tar cvjf ${bin_pkg} ${pnoslash}/bin/*.dll ${pnoslash}/share/doc/Cygwin
  # geos-devel
  tar cvjf ${bin_pkg2} ${pnoslash}/bin/geos-config \
    ${pnoslash}/include \
    ${pnoslash}/lib \
    ${pnoslash}/share/doc/${BASEPKG} )
}
# poor mans setup
