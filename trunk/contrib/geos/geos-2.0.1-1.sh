...
rsync_up_target="xarch:software/cygwin/gis"
...
conf() {
  (cd ${srcdir} && \
  CC=c++ CXX=c++ CFLAGS="${MY_CFLAGS}" LDFLAGS="${MY_LDFLAGS} -lstdc++" \
  ${srcdir}/configure \
...
pkg() {
  ...
  echo "" && \
  echo "tar -C / -jxvf ${FULLPKG}.tar.bz2" && \
  post="/etc/postinstall/${PKG}.sh" \
  echo "test -f ${post} && ${post} && mv ${post} ${post}.done" )
}
# poor mans setup
...
up() {
  (cd ${topdir} && \
    FILES="README.geos $tscriptname.sh ${src_patch_name} $src_pkg_name $bin_pkg_name" && \
    for f in $FILES; do \
      if [ ! -e $f ]; then if [ -e ${BASEPKG}/CYGWIN-PATCHES/$f ]; then \
        ln -s ${BASEPKG}/CYGWIN-PATCHES/$f $f; \
      fi; fi; \
    done && \
    for f in geos-get.sh ${src_pkg_name}.sig ${bin_pkg_name}.sig ${srcinstdir}/${FULLPKG}-log.tar.bz2; do \
      if [ -e $f ]; then FILES="${FILES} $f"; fi; done && \
    rsync -azL $FILES ${rsync_up_target}/${PKG}/ && \
    rsync -azL geos-setup.hint ${rsync_up_target}/${PKG}/setup.hint && \
