...
export checkfile=${srcinstdir}/${FULLPKG}.check
# run on
host=i686-pc-cygwin
...
prep() {
  ...
  chmod u+w ${srcdir}/GUI.rc && \
  chmod +x ${srcdir}/Win32-GUI-Scintilla/Include/SciLexer.dll
  chmod u+w ${srcdir}/samples/*
  mkdirs )
}
conf() {
  (cd ${srcdir} && \
...
while test -n "$1" ; do
  ...
  all) prep && conf && build && test && install && \
