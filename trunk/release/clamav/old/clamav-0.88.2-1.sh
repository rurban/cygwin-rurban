...
fixup() {
  # originally still libtool-1.4.2 but this doesn't work for cygwin with shared libs
  #alternatives --set libtool  /usr/bin/libtool-1.9f
  #alternatives --set automake /usr/bin/automake-1.9
  # shared lib patch by Brian Brubs (TBD: upstream)
  # need to remove libtool.m4 from acinclude.m4
  (cd ${srcdir}
...
#       only sendmail. fetchmail? procmail? ssmtp? email? nail? uw-mail?
conf() {
  (cd ${objdir} && \
  CFLAGS="${MY_CFLAGS}" LDFLAGS="-Wl,--enable-auto-image-base -Wl,--enable-runtime-pseudo-reloc ${MY_LDFLAGS}" \
  ${srcdir}/configure \
...
check() {
  ...
  # freshclam.conf already patched by prep. run with latest daily.cvd
...
up() {
  (cd ${topdir} && \
    mv $src_pkg_name $bin_pkg_name /usr/src/setup/release/${PKG}/
    cp ${srcdir}/CYGWIN-PATCHES/README setup.hint /usr/src/setup/release/${PKG}/
    mksetup setup && mksetup -v --noupset up )
}
finish() {
  rm -rf ${srcdir}
}
...
while test -n "$1" ; do
  case $1 in
    ...
    all)		checksig && prep && conf && build && test && install && \
