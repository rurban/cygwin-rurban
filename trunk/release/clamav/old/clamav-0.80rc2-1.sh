...
rsync_up_target="xarch:software/cygwin"
...
conf() {
  ...
  ${srcdir}/configure \
  --with-user=18 --with-group=18 \
  --srcdir=${srcdir} --prefix="${prefix}" \
  --exec-prefix='${prefix}' --sysconfdir="${sysconfdir}" \
  --libdir='${prefix}/lib' --includedir='${prefix}/include' \
  --mandir='${prefix}/share/man' --infodir='${prefix}/share/info' \
  --libexecdir='${sbindir}' --localstatedir="${localstatedir}" \
  --with-docdir="${prefix}/share/doc/${PKG}" --datadir='${prefix}/share' )
}
reconf() {
  (cd ${topdir} && \
  rm -fr ${objdir} && \
...
install() {
  ...
  /usr/bin/install -D -m 750 ${srcdir}/CYGWIN-PATCHES/clamd.initd ${instdir}/etc/rc.d/init.d/clamd && \
...
# mv ${instdir}/usr/lib/*.dll ${instdir}/usr/bin/ && \
  ...
  for d in ${prefix}/share/doc/${PKG} ${prefix}/share/doc/Cygwin ; do \
    if [ ! -d ${instdir}${d} ] ; then \
      mkdir -p ${instdir}${d} ;\
    fi ;\
  done &&\
  cp -r ${srcdir}/docs/html ${srcdir}/docs/*.pdf ${instdir}${prefix}/share/doc/${PKG} && \
...
up() {
  (cd ${topdir} && \
    rsync -avz setup.hint $tscriptname.sh $src_pkg_name $bin_pkg_name *.sig ${rsync_up_target}/${PKG}/ )
}
finish() {
  rm -rf ${srcdir}
}
