...
# $Id: generic-build-script,v 1.26 2004/08/12 21:19:39 igor Exp $

#custom vars
#where to drop the cygwin packages
rsync_up_target="xarch:software/cygwin/contrib/gdal"
...
export PKG2=gdal-devel
export PKG3=gdal-python
...
# de facto patch (includes fixup: libtoolize autoreconf)
export src_patch_name=${FULLPKG}.patch
# small difforig patch (for upstream)
export src_patch_devname=${BASEPKG}-${REL}-dev.patch
...
# some must run configure in srcdir, some have no configure at all
export objmakedir=${srcdir}
...
if [ -z "$MY_LDFLAGS" ]; then
  MY_LDFLAGS=-Wl,--enable-runtime-pseudo-reloc
fi
...
conf() {
  ...
  fixup && \
# --with-xerces --
  INCLUDE_OGR_FRMTS=yes CFLAGS="${MY_CFLAGS}" LDFLAGS="${MY_LDFLAGS}" \
  ${srcdir}/configure \
  --with-sqlite --with-libtiff=internal --with-geotiff=internal \
  ...
  cp ${objmakedir}/config.log ${objmakedir}/config.status ${srcinstdir}/ )
}
# try autoupdate and autoreconf?
fixup() {
  (cd ${srcdir} && \
...
build() {
  (cd ${objmakedir} && \
  CFLAGS="${MY_CFLAGS}" make 2>&1 | tee ${srcinstdir}/make.log  )
}
check() {
  (cd ${objmakedir} && \
...
install() {
  ...
  # fixup buggy ltmain.sh -o <noext>
  if [ -e ${instdir}/usr/lib/cyggdal-1 ]; then \
    mv ${instdir}/usr/lib/cyggdal-1 ${instdir}/usr/bin/cyggdal-1.exe; fi && \
...
pkg() {
  (cd ${instdir} && \
  # gdal
  tar cvjf ${bin_pkg} ${pnoslash}/bin/*.dll ${pnoslash}/bin/*.exe \
    ${pnoslash}/share \
    --exclude="${pnoslash}/share/man/man1/*.py*";
  # gdal-devel
  tar cvjf ${bin_pkg2}
    ${pnoslash}/include \
    ${pnoslash}/lib \
    --exclude="${pnoslash}/lib/python*";
  # gdal-python
  tar cvjf ${bin_pkg3} ${pnoslash}/bin/*.py ${pnoslash}/share/man/man1/*.py*
    ${pnoslash}/lib/python*; )
}
# poor mans setup
...
mkpatch() {
  ...
  cd ${topdir} && \
  diff -urN -x '.build' -x '.inst' -x '.sinst' -x '*~' -x '*.orig' \
...
spkg() {
  ...
  cd ${srcinstdir} && \
  if [ -e make.log ]; then \ 
    FILES="*.log config.status"; \
    tar cvjf ${FULLPKG}-log.tar.bz2 ${FILES} && rm ${FILES}; \
  fi; && \
  mkpatchd && cp ${topdir}/${src_patch_name} ${srcinstdir}/${src_patch_devname} && \
  if [ "${SIG}" -eq 1 ] ; then \
  ...
  if [ "${SIG}" -eq 1 ] ; then \
    cp ${topdir}/${tscriptname}.sh.sig ${srcinstdir}/ ; \
  fi && \
  cd ${srcinstdir} && \
  tar cvjf ${src_pkg} * )
}
up() {
  (cd ${topdir} && \
    FILES="README.${PKG} ${PKG}.hint $tscriptname.sh ${src_patch_name} $src_pkg_name $bin_pkg_name" && \
    for f in $FILES; do \
      if [ ! -e $f ]; then if [ -e ${BASEPKG}/CYGWIN-PATCHES/$f ]; then \
        ln -s ${BASEPKG}/CYGWIN-PATCHES/$f $f; \
      fi; fi; \
    done && \
    for f in get.sh ${PKG}-get.sh ${src_pkg_name}.sig ${bin_pkg_name}.sig ${srcinstdir}/${FULLPKG}-log.tar.bz2; do \
      if [ -e $f ]; then FILES="${FILES} $f"; fi; done && \
    rsync -avzL $FILES ${rsync_up_target}/${PKG}/ && \
    rsync -azL ${PKG}.hint ${rsync_up_target}/${PKG}/setup.hint && \
    \
    ln -sf ${BASEPKG}/CYGWIN-PATCHES/${PKG2}.hint setup.hint
    FILES="$bin_pkg_name2" && \
    for f in $FILES; do \
      if [ ! -e $f ]; then if [ -e ${BASEPKG}/CYGWIN-PATCHES/$f ]; then \
        ln -s ${BASEPKG}/CYGWIN-PATCHES/$f $f; \
    ...
    for f in README.${PKG2} ${PKG2}-get.sh ${bin_pkg_name2}.sig; do \
      if [ -e $f ]; then FILES="${FILES} $f"; fi; done && \
    rsync -azL $FILES ${rsync_up_target}/${PKG2}/ && \
    rsync -azL ${PKG2}.hint ${rsync_up_target}/${PKG2}/setup.hint && \
    \
    ln -sf ${BASEPKG}/CYGWIN-PATCHES/${PKG3}.hint setup.hint
    FILES="$bin_pkg_name3" && \
    for f in $FILES; do \
      if [ ! -e $f ]; then if [ -e ${BASEPKG}/CYGWIN-PATCHES/$f ]; then \
        ln -s ${BASEPKG}/CYGWIN-PATCHES/$f $f; \
    ...
    for f in README.${PKG3} ${PKG3}-get.sh ${bin_pkg_name3}.sig; do \
      if [ -e $f ]; then FILES="${FILES} $f"; fi; done && \
    rsync -azL $FILES ${rsync_up_target}/${PKG3}/ && \
    rsync -azL ${PKG3}.hint ${rsync_up_target}/${PKG3}/setup.hint && \
    \
    ssh $(echo $rsync_up_target| cut -d: -f1) ls -al $(echo $rsync_up_target| cut -d: -f2)/${PKG} && \
    ssh $(echo $rsync_up_target| cut -d: -f1) ls -al $(echo $rsync_up_target| cut -d: -f2)/${PKG2} && \
    ssh $(echo $rsync_up_target| cut -d: -f1) ls -al $(echo $rsync_up_target| cut -d: -f2)/${PKG3} && \
    FILES= && \
    true )
}
finish() {
