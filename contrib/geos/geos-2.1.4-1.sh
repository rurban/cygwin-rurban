...
rsync_up_target="xarch:software/cygwin/contrib/geos"
...
export objmakedir=${srcdir}/.build
...
# do autoupdate and autoreconf, but not aclocal
fixup() {
  (cd ${srcdir}
...
conf() {
  (cd ${objmakedir} && \
  CC=c++ CXX=c++ CXXFLAGS="-O2" CFLAGS="${MY_CFLAGS}" LDFLAGS="${MY_LDFLAGS} -lstdc++" \
  ${srcdir}/configure \
  ...
  cp ${objmakedir}/config.log ${srcinstdir}/ )
  # --docdir="${prefix}/share/doc/${BASEPKG}"
}
# try autoupdate and autoreconf?
# it wants automake-1.7
reconf() {
  fixup
}
reconf_hard() {
...
# doxygen exits with some errorlevel
  (cd doc; \
    cygstart doxygen Doxyfile | tee -a ${srcinstdir}/make-install.log 2>&1; \
    cd .. ) && \
# some hacks
  mkdir -p ${instdir}/usr/lib/libgeos && \
  mv ${instdir}/usr/bin/XMLTester.exe ${instdir}/usr/lib/libgeos/ && \
  ...
  mkdir -p ${instdir}/usr/share/doc/${BASEPKG}/html && \
  cp -r ${objmakedir}/doc/doxygen_docs/* ${instdir}/usr/share/doc/${BASEPKG}/html/ && \
  )
}
strip() {
  (cd ${instdir} && \
...
pkg() {
  ...
  set -x
  # geos
  tar cvjf ${bin_pkg} ${pnoslash}/bin/*.dll ${pnoslash}/lib/libgeos/*.exe ${pnoslash}/share/doc
  # geos-devel
  tar cvjf ${bin_pkg2} --exclude="${pnoslash}/lib/libgeos/*.exe" \
    ${pnoslash}/bin/geos-config \
    ${pnoslash}/include ${pnoslash}/lib )
}
# poor mans setup
...
up() {
  (cd ${topdir} && \
    ln -sf ${BASEPKG}/CYGWIN-PATCHES/README README.${PKG}
...
while test -n "$1" ; do
  case $1 in
    ...
    all)		checksig && prep && reconf && conf && build && install && \
