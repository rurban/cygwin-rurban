...
rsync_up_target="rurban.xarch.at:software/cygwin/release"
PATH=/bin:/usr/sbin:/usr/local/sbin:/usr/bin:/usr/local/bin:/usr/X11R6/bin
# ensure MSVC settings dont conflict
LIB=
INCLUDE=
# FIXED iodbcunix conflicts in /usr/include/iodbcunix.h
...
# dependencies
export dep1_zip=Win32-TieRegistry-0.25.zip
export dep2_tgz=Win32API-Registry-0.28.tar.gz
export dep1=Win32-TieRegistry-0.25
export dep2=Win32API-Registry-0.28
...
unpack() {
  ...
  find ${BASEPKG} -type f -exec chmod +w \{\} \; -exec dos2unix \{\} \;
...
prep() {
  ...
  mkdir ${BASEPKG} && \
  unzip -a $dep1_zip && mv "${dep1}" ${BASEPKG}/ && \
  tar xfz $dep2_tgz && mv "${dep2}" ${BASEPKG}/ && \
  unpack ${src_orig_pkg} && \
  cd ${topdir} && \
  if [ -f ${src_patch} ] ; then \
...
build() {
  (cd ${srcdir} && \
    make INSTALLDIRS=vendor all manifypods  | \
    tee ${srcinstdir}/make.log 2>&1 \
#  cd ${srcdir}/$dep1 && \
#    make INSTALLDIRS=vendor all manifypods  | \
#    tee -a ${srcinstdir}/make.log 2>&1 && \
#  cd ${srcdir}/$dep2 && \
#    make INSTALLDIRS=vendor all manifypods | \
#    tee -a ${srcinstdir}/make.log 2>&1 \
  )
...
#  cd ${srcdir}/$dep1 && \
#  make ${test_rule} | tee -a ${checkfile} 2>&1 && \
#  cd ${srcdir}/$dep2 && \
#  make ${test_rule} | tee -a ${checkfile} 2>&1 \
  )
}
...
install() {
  rm -rf ${instdir}/*; \
  (cd ${srcdir} && \
  mkdir -p ${instdir}/usr/share/man/man1 ${instdir}/usr/share/man/man3 && \
...
rebase1() {
  ...
  `cygrunsrv -L` > .services
  echo stopping services...
  for s in `cygrunsrv -L`; do cygrunsrv -E $s; done
  echo /bin/ps.exe -as | perl -ane 'print $F[0] if m/bin\/(bash|sh)/i' > .killallsh
  echo cygstart ash
  echo ./.killallsh
  echo rebaseall -v
  echo kill ash, start cygwin.bat
  echo continue with $0 rebase2
  )
}
rebase2() {
  (cd ${instdir}
  ...
  for s in `cat .services`; do cygrunsrv -S $s; done
  rm .services .killallsh
  set +x )
}
difforig() {
  (cd ${topdir} && \
  ...
  do
    ...
    diff -uN $FILE.orig $FILE # > ${topdir}/${src_patch_name}
  done
  ...
  do
    ...
    echo "$NEW" 1>&2
    diff -uN $FILE $NEW # > ${topdir}/${src_patch_name}
  done )
}
mkpatchd() {
  cp -f ${src_patch_name} ${src_patch_name}.bak
...
spkg() {
  ...
  cp ${dep1_zip} ${srcinstdir}/ && \
  cp ${dep2_tgz} ${srcinstdir}/ && \
...
while test -n "$1" ; do
  ...
  all) prep && conf && build && install && \
       check && strip && pkg && clean && spkg && finish ; \
	  STATUS=$? ;;
